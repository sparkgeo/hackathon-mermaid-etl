...
async def upsert(to_insert: Dict[str, object]) -> DbConstraintDetail:
    try:
        upsert = insert(site).values(to_insert).on_conflict_do_update(
...
async def validate(to_insert: Dict[str, object]) -> List[ValidationResult]:
    ...
    similar_sites = await db.fetch_all(
        select([
            ...
            geofunc.ST_Distance(site.c.location, to_insert[site.c.location.name]).label("distance"),
        ]).select_from(
            site
        ).where(
            and_(
                ...
                geofunc.ST_DWithin(site.c.location, to_insert[site.c.location.name], 100), # metres
            )
        )
    )
    similar_site_report = [ f"{site_data['name']} ({site_data['distance']}m)" for site_data in [dict(similar_site) for similar_site in similar_sites]]
    validation_results.append(ValidationResult(
        column_names=[],
        description="" if len(similar_site_report) == 0 else "Similar site(s) nearby: {0}".format(", ".join(similar_site_report)),
