...
def upgrade():
    ...
    sa.Column('created_on', sa.DateTime(), server_default=sa.text("(now() at time zone 'utc')"), nullable=False),
    sa.Column('updated_on', sa.DateTime(), server_default=sa.text("(now() at time zone 'utc')"), nullable=False),
    ...
    sa.Column('name', sa.String(length=255), nullable=False),
    sa.Column('location', geoalchemy2.types.Geography(geometry_type='POINT', from_text='ST_GeogFromText', name='geography'), nullable=False),
    sa.Column('notes', sa.Text(), nullable=False),
...
def downgrade():
    op.create_table('table_01',
    sa.Column('id', sa.SMALLINT(), server_default=sa.text("nextval('table_01_id_seq'::regclass)"), autoincrement=True, nullable=False),
